...
Создаем переменные рабочего окружения
...
#### PROJECT
Основной проект, название директории проекта, см. DATA_HOSTS

#### UID, GID
ID пользователя/группы, под которым вы работаете
```
$ id -u; $ id -g
```
...
#### MYSQL_ROOT_PASSWORD
Root пароль Mysql, под которым произойдет инициализация БД.  

#### RABBITMQ_USER, RABBITMQ_PASSWORD
Логин пароль к сервису очередей RabbitMQ

#### CONF_HOSTS
Конфигурационные файлы виртуальных хостов.
По умолчанию доступен виртуальный хост magicpro.
Укажите пусть к произвольной директории и создайте необходимое кол-во виртуальных хостов.
Например:
> CONF_HOSTS=./data/config/hosts/

#### DATA_HOSTS
Файлы проектов определяемых виртуальных хостов.
Например:
> DATA_HOSTS=./data/www/

В указанной директории должны находиться директории проектов:
- magipro/
- symfony/
- shop/
- test/
